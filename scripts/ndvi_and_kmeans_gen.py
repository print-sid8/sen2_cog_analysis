...
class CreateNDVI_and_KMeans():
    ...
    def get_sen2a_images(self,items,cloud_cover):
    ...
    def create_kmeans_output(self,ndvi_array,transform_info,crs,date,index):
        ...
        with rio.open(f"{outfolder}Unsupervised_KMeans_{date}_{self.payload['aois']['features'][index]['properties']['id']}.tif","w", **ras_meta) as dst:
            dst.write(out_dat,1)

        out_tiff_file = f"{outfolder}Unsupervised_KMeans_{date}_{self.payload['aois']['features'][index]['properties']['id']}.tif"
